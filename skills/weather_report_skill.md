...
3. **Escritura del Archivo (Herramienta de Infraestructura de Disco):**
   - Utiliza la herramienta de infraestructura `save_markdown_report` para guardar el contenido del reporte Markdown generado en el disco (pasando el nombre del archivo en `filename` y el contenido completo en `content`).
...
5. **Notificación al Usuario:** En tu respuesta final en el chat, notifica al usuario que has activado con éxito tu "Habilidad de Reportes Climáticos", muestra una previsualización atractiva del Markdown y confírmale la ruta exacta del archivo guardado.
